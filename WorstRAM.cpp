...
//最坏适应法
//分配内存接口
bool WorstRAM::AllocateMemory(Process process) {
    int maxSize = 0;
    //遍历分区向量，找到最大合分区
    for (auto &partitionSpace: partitionSpacesMap) {
        if (partitionSpace.isFree && partitionSpace.assignedSize >= process.needSize && partitionSpace.assignedSize > maxSize) {
            maxSize = partitionSpace.assignedSize;
        }
    }
    //如果找到合适的分区
    if (maxSize >= 0) {
        //分配内存
        for (auto &partitionSpace: partitionSpacesMap) {
            if (partitionSpace.isFree && partitionSpace.assignedSize == maxSize) {
...
//释放内存
void WorstRAM::RunOnceAndRecycle(int id) {
    //遍历所有分区
    for (auto &partitionSpace: partitionSpacesMap) {
        //如果是id的进程则释放
...
//显示内存分配状态
void WorstRAM::ShowMemoryStatus() {
    cout << "当前内存分配状态为：" << endl;
    for (auto &partitionSpace: partitionSpacesMap) {
...
//合并一次空闲分区
void WorstRAM::mergeFreePartitionSpacesOnce() {
    //遍历分区表
