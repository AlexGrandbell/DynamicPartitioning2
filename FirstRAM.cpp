...
//最先适应法
//分配内存接口
bool FirstRAM::AllocateMemory(Process process) {
    //遍历分区向量，找到合适的分区插入
    for (auto &partitionSpace: partitionSpacesMap) {
        if (partitionSpace.isFree && partitionSpace.assignedSize >= process.needSize) {
            //分配内存
...
//释放内存
void FirstRAM::RunOnceAndRecycle(int id) {
    //遍历所有分区
    for (auto &partitionSpace: partitionSpacesMap) {
        //如果是id的进程则释放
...
//显示内存分配状态
void FirstRAM::ShowMemoryStatus() {
    cout << "当前内存分配状态为：" << endl;
    for (auto &partitionSpace: partitionSpacesMap) {
...
//合并一次空闲分区
void FirstRAM::mergeFreePartitionSpacesOnce() {
    //遍历分区表
